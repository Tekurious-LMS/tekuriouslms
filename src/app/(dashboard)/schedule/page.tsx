...
const scheduleData: Record<
  string,
  { subject: string; status: "Scheduled" | "Running" | "Completed" }
> = {
    ...
    <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground">
            Weekly timetable and attendance.
          </p>
        </div>
      ...
      <div className="flex-1 border rounded-lg overflow-hidden bg-background shadow-sm">
        <div className="grid grid-cols-6 h-full divide-x divide-y">
          {/* Header Row */}
          <div className="bg-muted/50 p-4 font-semibold text-center text-sm flex items-center justify-center">
            Time
          </div>
          {weekDays.map((day) => (
            <div
              key={day}
              className="bg-muted/50 p-4 font-semibold text-center text-sm flex items-center justify-center"
            >
              {day}
            </div>
          ))}
          ...
          {timeSlots.map((time, timeIndex) => (
            <>
              <div
                key={`time-${timeIndex}`}
                className="p-2 text-xs text-muted-foreground font-medium flex items-start justify-center pt-4 bg-slate-50/50 dark:bg-slate-900/30"
              >
                {time}
              </div>
