...
import {
  ClassItem,
  ClassManagement,
} from "@/components/admin/structure/ClassManagement";
import {
  SubjectItem,
  SubjectManagement,
} from "@/components/admin/structure/SubjectManagement";
import {
  ClassSubjectMap,
  ClassSubjectMapping,
} from "@/components/admin/structure/ClassSubjectMapping";
...
export default function AcademicStructurePage() {
  ...
  const handleUpdateClass = (id: string, name: string, isActive: boolean) => {
    setClasses(
      classes.map((c) => (c.id === id ? { ...c, name, isActive } : c)),
    );
  };
  ...
  const handleUpdateSubject = (id: string, name: string, isActive: boolean) => {
    setSubjects(
      subjects.map((s) => (s.id === id ? { ...s, name, isActive } : s)),
    );
  };
  ...
  const handleUpdateMapping = (classId: string, subjectIds: string[]) => {
    ...
    if (existing) {
      setMappings(
        mappings.map((m) => (m.classId === classId ? { ...m, subjectIds } : m)),
      );
    } else {
  ...
  const totalMappings = mappings.reduce(
    (acc, curr) => acc + curr.subjectIds.length,
    0,
  );
    ...
    <div className="w-full min-h-screen bg-background p-4 sm:p-6 space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Define the educational hierarchy for your organization. Configure
          grades, subjects, and their relationships.
        </p>
      </div>
