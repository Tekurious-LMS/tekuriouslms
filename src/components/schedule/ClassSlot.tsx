...
export function ClassSlot({ subject, time, status, onClick }: ClassSlotProps) {
  const statusStyles = {
    ...
    Running:
      "border-l-4 border-l-green-500 bg-green-50 dark:bg-green-900/10 cursor-pointer hover:shadow-md transition-shadow",
    Completed:
      "border-l-4 border-l-slate-400 bg-slate-50 dark:bg-slate-900/10 opacity-70",
  };
  ...
  return (
    ...
    >
      ...
      <Badge
        variant="outline"
        className="w-fit text-[10px] h-5 mt-1 border-0 bg-white/50 dark:bg-black/20"
      >
        {status}
      </Badge>
    </Card>
