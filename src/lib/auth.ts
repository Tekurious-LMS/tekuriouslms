  ...
  hooks: {
    after: createAuthMiddleware(async (ctx) => {
      ...
      if (ctx.path === "/sign-up/email") {
        ...
        const { email, name, role } = ctx.body as {
          email: string;
          name: string;
          role: string;
        };
        ...
        console.log(
          `[AUTH HOOK] Extracted - email: ${email}, name: ${name}, role: ${role}`,
        );
        ...
        if (email && role) {
          ...
          console.log(
            `[AUTH HOOK] Found Better Auth user: ${betterAuthUser ? betterAuthUser.id : "NOT FOUND"}`,
          );
          ...
          if (betterAuthUser) {
            try {
              ...
              console.log(
                `[AUTH HOOK] Existing LmsUser: ${existingLmsUser ? existingLmsUser.id : "NONE"}`,
              );
              ...
              if (!existingLmsUser) {
                ...
                console.log(
                  `[AUTH HOOK] Created LmsUser: ${newLmsUser.id} for tenant: ${defaultTenant.slug}`,
                );
                ...
                if (roleRecord) {
                  ...
                  console.log(
                    `[AUTH HOOK] Assigned existing role ${role} (ID: ${roleRecord.id})`,
                  );
                } else {
                  ...
                  console.log(
                    `[AUTH HOOK] Created and assigned new role ${role} (ID: ${newRole.id})`,
                  );
                }
                console.log(
                  `[AUTH HOOK] Successfully created LmsUser and assigned role ${role} for ${email}`,
                );
              }
            } catch (error) {
          ...
          console.log(
            "[AUTH HOOK] Missing email or role, skipping LmsUser creation",
          );
        }
      }
      ...
      if (ctx.path === "/get-session" && ctx.context.session) {
        ...
        if (!lmsUser) {
          ...
          (session.user as SessionUser).lmsUserId = null;
          (session.user as SessionUser).role = null;
          (session.user as SessionUser).tenantId = null;
          (session.user as SessionUser).tenantSlug = null;
          (session.user as SessionUser).tenantName = null;
          return;
        }
        ...
        (session.user as SessionUser).lmsUserId = lmsUser.id;
        (session.user as SessionUser).role = primaryRole || null;
        (session.user as SessionUser).tenantId = lmsUser.tenant.id;
        (session.user as SessionUser).tenantSlug = lmsUser.tenant.slug;
        (session.user as SessionUser).tenantName = lmsUser.tenant.name;
      }
    }),
  },
});
