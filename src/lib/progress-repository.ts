 ...
 */
export async function getTeacherAnalytics(context: RBACContext): Promise<any> {
        ...
        courses.map(async (course) => {
            ...
            const assessmentParticipation =
                totalPossibleSubmissions > 0
                    ? Math.round(
                        (assessmentSubmissions / totalPossibleSubmissions) * 100
                    )
                    : 0;
 ...
 */
export async function getAdminAnalytics(context: RBACContext): Promise<any> {
